...
*/
	...
	public void addNewBusiness() throws InterruptedException {
		...
		waitUntil(driver -> {
			try {
				WebElement status = $(ScenarioView.class).first().policyStatus();
				return status != null && status.getText().equals("Active");
		...
		Assertions.assertEquals("Active",
				$(ScenarioView.class).first().policyStatus().getText());
	...
	/*
		...
		waitUntil(driver -> {
			try {
				return $(ScenarioView.class).first()
						.policyStatus()
						.getText()
						.equals("Active");
			} catch (StaleElementReferenceException | NoSuchElementException e) {
